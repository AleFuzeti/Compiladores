...
Set
    : SET H_VIEW Num COLON Num SEMICOLON{ float hhi = Get_Value($5); float hlo = Get_Value($3);
        if (hhi < hlo) printf("\nERROR: h_view_lo must be smaller than h_view_hi\n\n"); else { h_view_hi = hhi; h_view_lo = hlo; }
        initialize_plot(plot);}
    | SET V_VIEW Num COLON Num SEMICOLON{ float vhi = Get_Value($5); float vlo = Get_Value($3);
    ...
    | SET INTEGRAL_STEPS Num SEMICOLON { if ((int)Get_Value($3)<1) printf("\nERROR: integral_steps must be a positive non-zero integer\n\n"); 
...
Num 
    : INTEGER { $$ = Create_Node(INTEGER, NULL, NULL, (float)$1);}
    | REAL { $$ =  Create_Node(INTEGER, NULL, NULL, $1);}
    | MINUS REAL { $$ = Create_Node(INTEGER, NULL, NULL, -$2); }
    | MINUS INTEGER { $$ = Create_Node(INTEGER, NULL, NULL, (float)-$2); }
    | PLUS REAL { $$ = Create_Node(INTEGER, NULL, NULL, $2); }
    | PLUS INTEGER { $$ = Create_Node(INTEGER, NULL, NULL, (float)$2); }
    ;


Cons
    : PI { $$ = Create_Node(INTEGER, NULL, NULL, PI_); }
    | MINUS PI { $$ = Create_Node(INTEGER, NULL, NULL, -PI_); }
    | E { $$ = Create_Node(INTEGER, NULL, NULL, E_); }
    | MINUS E { $$ = Create_Node(INTEGER, NULL, NULL, -E_); }
    | X { $$ = Create_Node(X, NULL, NULL, 0); tem_x = 1; }
    | MINUS X { $$ = Create_Node(X, NULL, NULL, 0); tem_x = 1; }
    | IDENTIFIER { 
        float num = hash_table_search(ht, $1); 
            if (num == NOT_FIND) {
                //printf("Undefined symbol [%s]\n", $1);
