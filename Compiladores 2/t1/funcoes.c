...
matrix* mul_n_matrix(matrix* mat, float num){
    ...
    for (int i = 0; i < mat->rows; i++) {
        for (int j = 0; j < mat->cols; j++) {
            if(mat->data[i*mat->cols+j]  == 0) mul->data[i*mat->cols+j] = 0;
            else mul->data[i*mat->cols+j] = mat->data[i*mat->cols+j] * num;
        }
    }
    return mul;
}
...
void print_plot(char plot[HEIGHT][WIDTH]) {
    if (draw_axis) {
        int tem_x = 1, tem_y = 1, x_axis_position, y_axis_position;
        if(h_view_lo > 0 && h_view_hi < 0) tem_y = 0;
        if (v_view_lo > 0 && v_view_hi < 0) tem_x = 0;
        
        // Calcula a posição da linha do eixo X com base no intervalo vertical
        if (tem_x) {
            x_axis_position = (int)((v_view_hi / (v_view_hi - v_view_lo)) * (HEIGHT - 1));
            // Desenha o eixo X na posição calculada
            for (int i = 0; i < WIDTH; i++) {
                if (plot[x_axis_position][i] != '*') {
                    plot[x_axis_position][i] = '-';
                }
            }
        }

        // Calcula a posição da coluna do eixo Y com base no intervalo horizontal
        if (tem_y) {
            y_axis_position = (int)((-h_view_lo / (h_view_hi - h_view_lo)) * (WIDTH - 1));
            // Desenha o eixo Y na posição calculada
            for (int i = 0; i < HEIGHT; i++) {
                if (plot[i][y_axis_position] != '*') {
                    plot[i][y_axis_position] = '|';
                }
            }
        }
    }
