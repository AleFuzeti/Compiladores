...
TreeNode* Eval_Ast(TreeNode* aux){
    ...
    switch(aux->node_type){
        ...
        case SEN:
            ...
            aux->type = 'f';
            aux->value = aux->sinal * sin(esq->value);
            return aux;
            break;
        case COS:
            if (esq->type == 'm'){
            ...
            aux->type = 'f';
            aux->value = aux->sinal * cos(esq->value);
            return aux;
            break;
        case TAN:
            if (esq->type == 'm'){
            ...
            aux->type = 'f';
            aux->value = aux->sinal * tan(esq->value);
            return aux;
            break;
        case ABS:
            if (esq->type == 'm'){
            ...
            aux->type = 'f';
            aux->value = aux->sinal * abs(esq->value);
            return aux;
            break;
        case IDENTIFIER: 
            ...
            if (num == NOT_FIND) {
                ...
                if (aux->type == 'm'){
                    ...
                    if(aux->sinal == -1) aux->m = mul_n_matrix(aux->m, -1);
                    aux->value = 0;
                } else {
                    aux->value = num * aux->sinal;
                }
            return aux;
            break;
        case X : 
...
void RPN_Walk(TreeNode* aux){
    ...
    switch(aux->node_type){
        case INTEGER: print_formated_number(aux->value*aux->sinal,0); printf(" "); break;
        ...
        case IDENTIFIER: printf("%s ", aux->name); if(aux->sinal==-1)printf("NEG "); break;
        case X :  printf("x "); if(aux->sinal==-1)printf("NEG "); break;
        default:
            printf("Error ");
            break;
    }
...
float Get_Value(TreeNode* aux) {
    return aux->value * aux->sinal;
}

float Set_Value(TreeNode* aux, float value) {
    aux->value = value;
}
